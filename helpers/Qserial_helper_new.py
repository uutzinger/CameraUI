...
# Serial:         Model       - Functions running in separate thread, communication through signals and slots.
#
...
class QSerial(QObject):
    ...
    Signals (to be emitted by UI)
        ...
        closePortRequest                 request that Serial Worker closes current port
        displayingRunning                indicate that Serial Worker is running and want to display text in terminal
    ...
    Slots (functions available to respond to external signals)
        ...
        ---- 
        on_serialStatusReady                 pickup Serial Worker status on port, baudrate, line termination, timeout, connected
    ...
    def cleanup(self) -> None:
        """
        Perform cleanup tasks for QSerial, such as stopping timers, disconnecting signals,
...
class Serial(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.handle_log(
            logging.INFO,
            f"[{self.thread_id}]: Serial initialized."
        )
    ...
    def on_setupReceiverRequest(self) -> None:
        ...
        self.QSer.errorOccurred.connect(
            lambda err: (
                ...
                self.handle_log(
                    logging.ERROR,
                    f"[{self.thread_id}]: Serial Error {err}: {self.QSer.errorString()}"
                )
            )
        )
